...
class MarketplaceProject(Base):
    ...
    # Выбранный заказчиком мастер (после select-master). Имя денормализовано,
    # чтобы лента/деталь проекта не зависели от наличия профиля мастера.
    selected_master_id = Column(
        Integer,
        ForeignKey("users.id", ondelete="SET NULL"),
        nullable=True
    )
    selected_master_name = Column(String(160), nullable=True)
